 ...
 * 动作工厂类，负责创建各种动作处理器
 */
class ActionFactory(private val plugin: PostSpawner) {
    ...
    init {
        ...
        registerAction(ActionType.DROP_MONSTER_SPAWNER, DropMonsterSpawnerAction(plugin))
        registerAction(ActionType.DROP, DropAction(plugin))
        registerAction(ActionType.GIVE, GiveAction(plugin))
        registerAction(ActionType.BUFF, BuffAction(plugin))
        registerAction(ActionType.MONEY, MoneyAction(plugin))
        registerAction(ActionType.POINTS, PointsAction(plugin))
